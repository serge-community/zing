...
@pytest.mark.parametrize(
    "words_left, days_left, expected_factor",
    [
        (1500, 10, 0.15),
        (20, 10, 0.002),
        (5000, 10, 0.5),
...
def test_translation_task_importance_factor(words_left, days_left, expected_factor):
    """Tests `TranslationTask`'s importance factor."""
    now = aware_datetime(2017, 2, 28, 9, 0)
    due_on = now + timedelta(days=days_left)
    due_date = DueDateFactory.build(due_on=due_on)
    task = TranslationTask(due_date=due_date, now=now, words_left=words_left)
    assert task.importance_factor == expected_factor


@pytest.mark.parametrize(
    "words_left, days_left, expected_factor", [(1, 5, 0.2), (1, 1, 1), (20, 1, 1)]
)
...
def test_taskresultset_importance_order(
    test_name, snapshot_stack, sort_type, fake_task_args
):
