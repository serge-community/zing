...
class Command(UserCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "user", nargs=1, help="Username of account to merge from",
        )
        parser.add_argument(
            "other_user", nargs=1, help="Username of account to merge into",
        )
    ...
    def handle(self, **options):
        ...
        utils.UserMerger(
            src_user, self.get_user(username=options["other_user"][0])
        ).merge()
