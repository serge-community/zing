...
FAKE_EMAIL_CONFIG = dict(
    {"email": "foo@example.org", "accounting_email": "bar@example.com"}, **FAKE_CONFIG
)
...
@pytest.mark.parametrize(
    "month", [None, timezone.now(), timezone.make_aware(timezone.datetime(2014, 4, 1))]
)
...
@pytest.mark.parametrize(
    "config, require_email_fields",
    [
        ({}, False),
        ({"foo": None, "bar": False}, False),
        ({"name": None, "paid_by": None}, False),
        ({"name": None, "paid_by": None, "wire_info": None}, True),
        ({"name": None, "paid_by": None, "wire_info": None, "email": None}, True),
        (
            {
                "name": None,
                "paid_by": None,
...
def test_invoice_check_config_for(config, require_email_fields):
    with pytest.raises(ImproperlyConfigured):
        Invoice.check_config_for(
            config, "fake_username", require_email_fields=require_email_fields
        )
...
@pytest.mark.django_db
@pytest.mark.parametrize(
    "task_type, task_type_name, user_rate_attr_name",
    [
        (PaidTaskTypes.TRANSLATION, "TRANSLATION", "rate"),
        (PaidTaskTypes.REVIEW, "REVIEW", "review_rate"),
    ],
)
def test_invoice_get_rates_inconsistent_paidtask_rates(
    member, task_type, task_type_name, user_rate_attr_name, store0
):
    USER_RATE = 0.5
    PAID_TASK_RATE = 0.2
...
@pytest.mark.parametrize(
    "action_code", (TranslationActionCodes.NEW, TranslationActionCodes.REVIEWED)
)
def test_invoice_get_user_amounts(member, action_code, task_type):
    ...
    if (
        action_code == TranslationActionCodes.NEW
        and task_type == PaidTaskTypes.TRANSLATION
    ):
        assert translated == (EVENT_COUNT * WORDCOUNT + TASK_COUNT * PAID_TASK_AMOUNT)
    ...
    if (
        action_code == TranslationActionCodes.REVIEWED
        and task_type == PaidTaskTypes.REVIEW
    ):
        assert reviewed == (EVENT_COUNT * WORDCOUNT + TASK_COUNT * PAID_TASK_AMOUNT)
...
def test_invoice_generate_add_carry_over(member, invoice_directory):
    ...
    month = timezone.make_aware(timezone.datetime(2014, 4, 1))
    config = dict({"minimal_payment": MINIMAL_PAYMENT}, **FAKE_CONFIG)
