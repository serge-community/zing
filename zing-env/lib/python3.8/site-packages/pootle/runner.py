...
def init_settings(
    ...
    db_module = {"sqlite": "sqlite3", "mysql": "mysql", "postgresql": "postgresql"}[db]

    context = {
        "default_key": ("'%s'" % b64encode(os.urandom(KEY_LENGTH)).decode("utf-8")),
...
def init_command(parser, settings_template, args):
    ...
    parser.add_argument(
        ...
        help=(u"Use the specified database backend (default: " u"%(default)s)."),
    )
    parser.add_argument(
        "--db-name",
        default="",
        help=(
            u"Database name (default: 'zingdb') or path "
    ...
    parser.add_argument(
        "--db-user",
        default="",
        help=(u"Name of the database user. Not used with " u"sqlite."),
    )
    parser.add_argument(
        "--db-host",
        default="",
        help=(u"Database host. Defaults to localhost. Not " u"used with sqlite."),
    )
    parser.add_argument(
        "--db-port",
        default="",
        help=(u"Database port. Defaults to backend default. " u"Not used with sqlite."),
    )
    ...
    if os.path.exists(config_path):
        ...
        if args.noinput:
            ...
            resp = input(
                "File already exists at %r, overwrite? [Ny] " % config_path
            ).lower()
...
def configure_app(project, config_path, django_settings_module, runner_name):
    ...
    if not (os.path.exists(config_path) or os.environ.get(settings_envvar, None)):
        print(
            u"Configuration file does not exist at %r or "
...
def run_app(project, default_settings_path, settings_template, django_settings_module):
    ...
    parser.add_argument(
        "--noinput", action="store_true", default=False, help=u"Never prompt for input",
    )
    parser.add_argument(
        "--no-rq",
        action="store_true",
