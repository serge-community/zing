...
def tests_use_db(request):
    return bool(
        [item for item in request.node.items if item.get_closest_marker("django_db")]
    )
...
pytest_plugins = tuple(_load_fixtures(fixtures, fixtures_core_utils, fixtures_models,))
