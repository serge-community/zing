...
from pootle_store.constants import FUZZY, OBSOLETE, STATES_MAP, TRANSLATED, UNTRANSLATED
from pootle_store.fields import to_python
from pootle_store.models import Suggestion, QualityCheck, Unit
...
class ProxyTimelineUser(object):
    ...
    def get_absolute_url(self):
        return reverse(
            "pootle-user-profile", args=[self.submission["submitter__username"]]
        )
    ...
    def gravatar_url(self, size=80):
        ...
        return "https://secure.gravatar.com/avatar/%s?s=%d&d=mm" % (email_hash, size)


def _get_suggestion_description(submission):
    user_url = reverse(
        "pootle-user-profile", args=[submission["suggestion__user__username"]]
    )
    display_name = (
    ...
    if comment:
        sugg_accepted_desc = _(
            u"Accepted suggestion from %(author)s with comment: %(comment)s" % params
        )
        sugg_rejected_desc = _(
            u"Rejected suggestion from %(author)s with comment: %(comment)s" % params
        )
    else:
...
def _timeline_test(client, request_user, unit):
    ...
    if user.username != "nobody":
        client.login(username=user.username, password=request_user["password"])
    response = client.get(url, HTTP_X_REQUESTED_WITH="XMLHttpRequest")

    no_permission = not user.is_superuser and unit not in Unit.objects.get_translatable(
        user
    )
...
def test_timeline_view_units(client, request_users, system, admin):
    _timeline_test(client, request_users, Unit.objects.filter(state=TRANSLATED).first())
    _timeline_test(
        client, request_users, Unit.objects.filter(state=UNTRANSLATED).first()
    )


@pytest.mark.xfail(reason="timeline does not currently check permissions correctly")
@pytest.mark.django_db
def test_timeline_view_unit_obsolete(client, request_users, system, admin):
    _timeline_test(client, request_users, Unit.objects.filter(state=OBSOLETE).first())


@pytest.mark.xfail(reason="timeline does not currently check permissions correctly")
@pytest.mark.django_db
def test_timeline_view_unit_disabled_project(client, request_users, system, admin):
    unit = Unit.objects.filter(
        store__translation_project__project__disabled=True, state=TRANSLATED
    ).first()
...
def test_timeline_view_unit_with_suggestion(
    client, request_users, system, admin, store0, revision
):
...
@pytest.mark.django_db
def test_timeline_view_unit_with_creation(client, request_users, system, admin, store0):
