...
urlpatterns = [
    ...
    url(r"^xhr/uids/?$", views.get_uids, name="pootle-xhr-uids"),
    url(r"^xhr/units/?$", views.get_units, name="pootle-xhr-units"),
    url(r"^xhr/units/(?P<uid>[0-9]+)/?$", views.submit, name="pootle-xhr-units-submit"),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/comment/?$",
        views.comment,
        name="pootle-xhr-units-comment",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/context/?$",
        views.get_context_units,
        name="pootle-xhr-units-context",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/edit/?$",
        views.UnitEditJSON.as_view(),
        name="pootle-xhr-units-edit",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/timeline/?$",
        views.UnitTimelineJSON.as_view(),
        name="pootle-xhr-units-timeline",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/suggestions/?$",
        views.suggest,
        name="pootle-xhr-units-suggest",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/suggestions/(?P<sugg_id>[0-9]+)/?$",
        views.manage_suggestion,
        name="pootle-xhr-units-suggest-manage",
    ),
    url(
        r"^xhr/units/(?P<uid>[0-9]+)/checks/(?P<check_id>[0-9]+)/toggle/?$",
