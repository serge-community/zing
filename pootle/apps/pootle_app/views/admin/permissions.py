...
def admin_permissions(request, current_directory, template, ctx):
    ...
    choice_groups = [(None, base_queryset.filter(username__in=("nobody", "default")))]

    choice_groups.append(
        (
            _("All Users"),
            base_queryset.exclude(username__in=("nobody", "default")).order_by(
                "username"
            ),
        )
    )
    ...
    class PermissionSetForm(forms.ModelForm):
        ...
        user = GroupedModelChoiceField(
            ...
            required=True,
            widget=forms.Select(attrs={"class": "js-select2 select2-username"}),
        )
