...
@pytest.mark.parametrize(
    ...
    [(u"$1 aa $2", u"$1 dd $2", True), (u"$1 aa $2", u"$1dd$2", True)],
)
...
def test_plurr_format(source_string, target_string, should_skip):
    ...
    assert_check(check, source_string, target_string, should_skip, language_code="ru")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (u"{N}", u"{N::::}", True),
        (
            u"{N:foo {C}|{N_PLURAL:bar {N} in {C}|bar {N} baz {C}}}",
            u"{N:{C}foo|{N_PLURAL:{C}bar}}",
            True,
        ),
        ...
        (
            ...
            u"{CUOTA} se restablece en {N} {FOO_PLURAL:día|días}",
            False,
        ),
    ],
)
...
@pytest.mark.parametrize("fake_check_name", ["fake-name", None, 0])
